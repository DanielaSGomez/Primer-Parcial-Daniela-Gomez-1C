...
int anadirPeliculaaut(ePeliculas lista[], int tam, int id, char genero[], char actor[], char paisOrigen[],char titulo[])
{
    ...
    else
    {
        ...
        strcpy(lista[hayLugar].actor,actor);
        ...
        strcpy(lista[hayLugar].paisOrigen,paisOrigen);
...
{
    printf("------------------------------------------------------------------------------------------------");
    printf("\nID: %d\tTITULO: %s\tACTOR: %s \tFECHA DE ESTRENO: %d - %d -%d\n",unElemento.idPelicula,unElemento.titulo,unElemento.actor, unElemento.fecha.dia,unElemento.fecha.mes, unElemento.fecha.anio);
    printf("\n---------------------------------------------------------------------------------------------");
    return 0;
}
...
int mostrarListaElementos(ePeliculas lista[], int tam)
{
    ...
    for(i=0;i<tam;i++)
    {
        if(lista[i].isEmpty==OCUPADO)
        {
             ...
             printf("ID: %d\tTITULO: %s\tACTOR: %s \tFECHA DE ESTRENO: %d - %d -%d\n",lista[i].idPelicula,lista[i].titulo,lista[i].actor,lista[i].fecha.dia,lista[i].fecha.mes,lista[i].fecha.anio);
        }
                    ...
                    do
                    {
                        ...
                        switch(opcionMenuModificar)
                        {
                            ...
                            case 2:
                                ...
                                strcpy(lista[existeId].actor , nuevoApellido);
                                break;
                            case 3:

                                break;
                            case 4:
                                nuevoSector = getValidInt("\nIngrese el nueva fecha (0 a 50): ", "\nDebe ser un numero entero (0 a 50)",0,51);
                                lista[existeId].fecha.dia = nuevoSector;
                                nuevoSector = getValidInt("\nIngrese el nueva mes (0 a 50): ", "\nDebe ser un numero entero (0 a 50)",0,51);
                                lista[existeId].fecha.mes= nuevoSector;

                                nuevoSector = getValidInt("\nIngrese el nueva año (0 a 50): ", "\nDebe ser un numero entero (0 a 50)",0,51);
...
int harcodearData(ePeliculas lista[] ,int tam)
{
    ...
    for(i=0;i<tam;i++)
    {
        ...
        strcpy(lista[i].paisOrigen,pais[i]);
